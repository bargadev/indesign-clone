// @ts-nocheck
// Portado de tiptap-playground. Alças entre colunas (ligado ao editor de miolo ativo).
// Ajuste p/ zoom: usa rect.width (escalado) em vez de offsetWidth (layout) no cálculo.
import { useEffect, useState } from 'react'

const GAP = 18 // = gap do grid de colunas no index.css (.body-editor .cols-editable)
...
export default function ColumnResizers({ editor }) {
  ...
  const startResize = (e, bar) => {
    ...
    // escala atual = rect/layout; usable em px de TELA p/ casar com clientX
    const rectW = dom ? dom.getBoundingClientRect().width : 600
    const scale = dom && dom.offsetWidth ? rectW / dom.offsetWidth : 1
    const usable = rectW - (startW.length - 1) * GAP * scale
