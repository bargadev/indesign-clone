// @ts-nocheck
// Portado de tiptap-playground (blockDrag.js). Motor de arraste de bloco estilo Notion.
...
const createColumns = (editor, dragPos, dragNode, { targetPos, targetNode, side }) => {
  ...
  if ($t.parent.type.name === 'column') {
    ...
    if (columnsNode.childCount >= 3) return
...
export function startBlockDrag(editor, dragPos, dragNode, ev) {
  ...
  const onUp = () => {
    ...
    if (!drop) return
    if (drop.mode === 'col') createColumns(editor, dragPos, dragNode, drop)
    else if (drop.mode === 'outer') {
      const insertPos =
